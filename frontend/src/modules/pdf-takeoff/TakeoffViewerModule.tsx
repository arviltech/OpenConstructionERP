...
export default function TakeoffViewerModule({
  ...
  /** The browser fires `click` after `mouseup` for a press and release on the
   *  same element, so a gesture that has already been consumed must be able to
   *  swallow that trailing click or it places a point. `panRef` does this for a
   *  pan; this ref does it for an in-canvas edit drag that a tool switch
   *  aborted mid-press, whose mouseup would otherwise be read as the first
   *  click of a new measurement in the tool just selected. Cleared on the next
   *  mousedown so an abandoned flag (release off-canvas fires no click here)
   *  cannot swallow an unrelated click later. */
  const suppressNextClickRef = useRef(false);
  /** A canvas press is in flight (button down, not yet released). The trailing
   *  click belongs to the tool that was active when the press STARTED, so this,
   *  not `dragRef`, is what a mid-press tool switch has to key off: Escape
   *  aborts the drag directly while the button is still held, which clears
   *  `dragRef` but leaves the press (and its pending click) very much alive.
   *
   *  Mouse only, deliberately. An edit drag is armed in `handleCanvasMouseDown`,
   *  and a finger that moves across the canvas produces no compatibility mouse
   *  events, so a touch gesture never starts one and cannot reach this bug. */
  ...
  const handleCanvasClick = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      // and in sticky pan mode (#316) a click never places one either. An edit
      // drag aborted by a mid-press tool switch is consumed the same way: its
      // trailing click belongs to the finished drag, not to the new tool.
  ...
  const handleCanvasMouseDown = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      // Primary button only, for BOTH flags. `click` is only ever generated by a
      // primary press and release (a non-primary press fires `auxclick`), so a
      // primary press is the only thing that starts a new click sequence, and it
      // is the only thing entitled to reset this state. Clearing on any button
      // would let a middle-click (pan) tapped mid-press disarm a guard that a
      // tool switch had just armed, or drop a press that is still held.
      if (e.button === 0) {
        // A new press starts a new gesture, so any click still owed to the last
        // one is stale. Releasing an aborted drag outside the canvas fires no
        // click here, which would otherwise leave the flag armed indefinitely.
        suppressNextClickRef.current = false;
  ...
  useEffect(() => {
    ...
    const onUp = (e: MouseEvent) => {
      // The press is over wherever it was released, so a tool switch after this
      // point owns its own click and must not inherit this gesture's. Primary
      // button only, for the reason given where the flag is set.
      if (e.button === 0) pressActiveRef.current = false;
      if (panRef.current) {
        panRef.current = null;
        setPanning(false);
  ...
  const selectTool = useCallback((tool: MeasureTool) => {
    // The shortcut keys stay live while the mouse is held, so a tool switch can
    // land in the middle of a press. Two things then leak into the new tool.
    // An in-canvas edit drag would otherwise survive and commit on mouseup at a
    // position the canvas has already stopped previewing (the edit overlay runs
    // only under the select tool, so the shape visibly reverts), so abandon it,
    // matching what Escape already does for a reshape the user bails out of.
    // And the trailing click the browser fires for that press belongs to the
    // tool that was active when the press STARTED, so it must not place a point
    // in the tool just selected. That is keyed on the press, not on the drag:
    // Escape can abort the drag while the button is still down, leaving no drag
    // but a live press whose click would still leak.
    if (tool !== activeTool) {
  ...
  useEffect(() => {
    ...
    const blurHandler = () => {
      ...
      spaceHeldRef.current = false;
      setSpaceHeld(false);
      // The mouse gesture is stranded by a blur for the same reason. A button
      // released in another application delivers no `mouseup` here, so an edit
      // drag stays armed and the next ordinary mousemove goes on reshaping the
      // measurement with nothing held down. The press flags leak with it.
      if (dragRef.current) finishDrag(false);
      // A pan is stranded the same way. `keyup` on Space ends one cleanly, but a
      // blur delivers neither that nor a `mouseup`, so `panRef` survives and the
      // next bare mousemove goes on scrolling the sheet. `panLock` is a mode, not
      // a gesture, so it stays.
      if (panRef.current) {
        panRef.current = null;
        setPanning(false);
      }
